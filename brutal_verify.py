...
for path in sorted(HQ.glob("*.jsonl")):
    ft = (
        "trigger_bot"
        if path.name.endswith("trigger_bot.jsonl")
        else (
            "welcome_click_start"
            if path.name.endswith("welcome_click_start.jsonl")
            else "flw_registration" if path.name.endswith("flw_registration.jsonl") else None
        )
    )
    if not ft:
        continue
...
try:
    ...
    with open("connect_user_data_snapshot.csv", encoding="utf-8", errors="replace") as _sf:
        for _sr in _csv.DictReader(_sf):
            _iv = (_sr.get("invited_date") or "")[:10]
            if _iv:
                _snap_dates.append(_iv)
except OSError:
    pass
_snap_newest = max(_snap_dates) if _snap_dates else ""
_snap_age = (TODAY - date.fromisoformat(_snap_newest)).days if _snap_newest else 9999
chk(
    "connect snapshot CONTENT is recent (newest invited_date, not file mtime)",
    _snap_age <= 45,
...
sched = (
    json.loads((ROOT / "_interview_schedule.json").read_text(encoding="utf-8"))
    if (ROOT / "_interview_schedule.json").exists()
    else {}
)
seen = {}
for cid, seq in sched.items():
    ...
    if not sg or is_test(cid) or sg in seen:
        continue
    seen[sg] = ([s["topic"] for s in seq], (seq[1]["offset_days"] - seq[0]["offset_days"]) if len(seq) > 1 else None)
for sg, (topics, cad) in seen.items():
    chk(
        f"design[{sg}].topics == lookup",
        DESIGN.get(sg, {}).get("topics") == topics,
        f"{DESIGN.get(sg, {}).get('topics')} vs {topics}",
    )
    if cad is not None:
        chk(
            f"design[{sg}].cadence == lookup",
            DESIGN.get(sg, {}).get("cadence") == cad,
...
valid_trig = [
    ...
    if cohort_to_sg(tf[1]) and not is_test(tf[1]) and tf[2] in DESIGN.get(cohort_to_sg(tf[1]), {}).get("topics", [])
]
chk("master rows == valid trigger forms (raw)", len(R) == len(valid_trig), f"{len(R)} == {len(valid_trig)}")
chk(
    "master unique FLWs == unique cids in valid triggers (raw)",
...
chk(
    "counts.cohorts == mapped cohorts (Connect snapshot ∪ CommCare interview data)",
    DD["counts"]["cohorts"] == len(union_cohorts),
    f"{DD['counts']['cohorts']} == {len(union_cohorts)}",
)
...
for row in snap:
    ...
    if not u:
        continue
    if inv:
        raw_fun[sg]["invited"].add(u)
    if (row.get("user_invite_status") or "").strip() == "accepted":
        raw_fun[sg]["accepted"].add(u)
    if pdt(row.get("completed_learn_date")):
        raw_fun[sg]["learn_completed"].add(u)
    if pdt(row.get("date_claimed")):
        raw_fun[sg]["claimed"].add(u)
cf = {r["sg"]: r for r in DD["connectFunnel"]}
for sg in cf:
    for fld in ["invited", "accepted", "learn_completed", "claimed"]:
        chk(
            f"connectFunnel[{sg}].{fld} == raw snapshot",
            cf[sg][fld] == len(raw_fun[sg][fld]),
...
chk(
    "counts.started == unique started interviews (cells)",
    DD["counts"]["started"]
    == len({(r["connect_id"], r["cohort_id"], r["interview_n"]) for r in R if r["is_started"] == "Y"}),
    str(DD["counts"]["started"]),
)
chk(
    "counts.completed == unique completed interviews (cells)",
    DD["counts"]["completed"]
    == len({(r["connect_id"], r["cohort_id"], r["interview_n"]) for r in R if r["is_completed"] == "Y"}),
    str(DD["counts"]["completed"]),
)
_t1o = next((x for x in DD["table1"] if x["key"] == "Overall"), {})
chk(
    "counts.started == Breakdowns Overall ist (tie-out)",
    DD["counts"]["started"] == _t1o.get("ist"),
    f"{DD['counts']['started']} == {_t1o.get('ist')}",
)
chk(
    "counts.completed == Breakdowns Overall icmp (tie-out)",
    DD["counts"]["completed"] == _t1o.get("icmp"),
    f"{DD['counts']['completed']} == {_t1o.get('icmp')}",
)

# D-connectFunnel started/completed/initiated
sg_started = defaultdict(set)
sg_completed = defaultdict(set)
for r in R:
    if r["is_started"] == "Y":
...
for sg in cf:
    chk(
        f"connectFunnel[{sg}].started",
        cf[sg]["started"] == len(sg_started[sg]),
        f"{cf[sg]['started']} == {len(sg_started[sg])}",
    )
    chk(
        f"connectFunnel[{sg}].completed",
        cf[sg]["completed"] == len(sg_completed[sg]),
        f"{cf[sg]['completed']} == {len(sg_completed[sg])}",
    )
    chk(
        f"connectFunnel[{sg}].initiated",
        cf[sg]["initiated"] == len(init_sg[sg]),
        f"{cf[sg]['initiated']} == {len(init_sg[sg])}",
    )
...
for row in DD["table1"]:
    ...
    chk(
        f"table1[{k}].ist/icmp",
        row["ist"] == t1[k]["ist"] and row["icmp"] == t1[k]["icmp"],
...
for row in DD["table3"]:
    k = row["key"]
    chk(
        f"table3[{k}].flws/ist/icmp",
        row["flws"] == len(t3[k]["flw"]) and row["ist"] == t3[k]["ist"] and row["icmp"] == t3[k]["icmp"],
...
for row in DD["table2"]:
    tc = row["code"]
    chk(
        f"table2[{tc}].flws/ist/icmp",
        row["flws"] == len(t2[tc]["flw"]) and row["ist"] == t2[tc]["ist"] and row["icmp"] == t2[tc]["icmp"],
...
for r in R:
    ...
    if r["is_started"] == "Y":
        fset[key]["s"].add(r["connect_id"])
    if r["is_completed"] == "Y":
        fset[key]["c"].add(r["connect_id"])
...
for sg, s in do.items():
    ...
    for iv in s["interviews"]:
        ...
        f = fset[(sg, n)]
        if not (
            iv["eligible"] == len(init_sg[sg])
            and iv["triggered"] == len(f["t"])
            and iv["started"] == len(f["s"])
            and iv["completed"] == len(f["c"])
        ):
            do_bad += 1
            print(
                f"    dropoff mismatch {sg} Int{n}: elig {iv['eligible']}/{len(init_sg[sg])} trig {iv['triggered']}/{len(f['t'])} start {iv['started']}/{len(f['s'])} compl {iv['completed']}/{len(f['c'])}"
            )
        # pct checks
        if iv["pct_started"] != tsl.r1(100 * len(f["s"]) / elig):
            do_bad += 1
        if iv["pct_completed_base"] != tsl.r1(100 * len(f["c"]) / elig):
            do_bad += 1
        exp_pc = tsl.r1(100 * len(f["c"]) / len(f["s"])) if f["s"] else None
        if iv["pct_completed"] != exp_pc:
            do_bad += 1
...
cohort_claimed = defaultdict(set)
cohort_sg = {}
for row in snap:
    u = (row.get("username") or "").strip()
    if u in EXCLUDE_FLWS:
...
for row in DD["topicStatus"]:
    ...
    for s in STATES6:
        if row[s] != ts_rec[tc][s]:
            ts_bad += 1
            print(f"    topicStatus {tc}.{s}: {row[s]} vs {ts_rec[tc][s]}")
    if row["total"] != sum(ts_rec[tc][s] for s in STATES6):
        ts_bad += 1
    if row["applicable"] != row["total"] - row["not-applicable"]:
        ts_bad += 1
...
chk(
    "flwMatrix rows == (claimed ∪ interviewed) (FLW,cohort) pairs (raw snapshot)",
    len(DD["flwMatrix"]) == claimed_pairs,
...
chk(
    "no started interview falls outside the matrix (Σtable2.ist == flwMatrix started cells)",
    _t2_ist == _mat_ist,
    f"{_t2_ist} == {_mat_ist}",
)
chk(
    "no completed interview falls outside the matrix (Σtable2.icmp == flwMatrix completed cells)",
    _t2_icmp == _mat_icmp,
    f"{_t2_icmp} == {_mat_icmp}",
)
...
chk(
    "flwMatrix completed cells == Σ topicStatus completed",
    mat_completed == ts_completed,
    f"{mat_completed} == {ts_completed}",
)
chk(
    "flwMatrix started cells == Σ topicStatus started+completed",
    mat_started == ts_started,
    f"{mat_started} == {ts_started}",
)
...
for sg in cf:
    ...
    for fld in ["invited", "accepted", "learn_completed", "claimed", "initiated"]:
        if cf[sg][fld] != dc.get(fld):
            cpe += 1
            print(f"    connectFunnel vs dropoff.connect {sg}.{fld}: {cf[sg][fld]} vs {dc.get(fld)}")
chk(
    "Overview connectFunnel == Funnels/FullRetention dropoff.connect (invited/accepted/learnC/claimed/initiated)",
    cpe == 0,
    f"{cpe} bad",
)
...
for sg in ["TRS", "TRE", "PANEL"]:
    ...
    if row and sg in cf:
        chk(
            f"table1[{sg}].flws == connectFunnel[{sg}].started (unique started FLWs)",
            row["flws"] == cf[sg]["started"],
            f"{row['flws']} == {cf[sg]['started']}",
        )
...
for s in DD["lineSeries"]:
    sg = s["sg"]
    if s["base"] != len(init_sg[sg]):
        ls_bad += 1
    for i, p in enumerate(s["pts"]):
        if fun.get((sg, i + 1)) and abs(fun[(sg, i + 1)]["pct_started"] - p) > 1e-9:
            ls_bad += 1
...
chk(
    "render_data.json substring present verbatim in injected render",
    raw_json in render,
...
chk(
    "render_data top-level keys == dashboard keys − allowlist + flwMatrix encoding",
    set(RD) == exp_top,
...
chk(
    "topicStatusCohort compression inverts exactly (every row, every state)",
    not _tsc_bad,
...
chk(
    "dropoff.subgroups byte-identical (subgroup-level connect/di/base ARE read by the render)",
    _j(DD["dropoff"]["subgroups"]) == _j(RD["dropoff"].get("subgroups")),
    "",
)
chk(
    "dropoff sibling keys preserved",
    set(DD["dropoff"]) == set(RD["dropoff"]),
...
chk(
    "dropoff.cohorts[] dropped keys == exactly {connect}",
    co_dropped_co == ALLOWED_DROP_COHORT,
    f"{sorted(co_dropped_co)}",
)
chk(
    "dropoff.cohorts[].interviews[] dropped keys == exactly the allowlist",
    co_dropped_iv == ALLOWED_DROP_COHORT_IV,
    f"{sorted(co_dropped_iv)}",
)
...
chk(
    "flwMatrixV2 decodes to the same row COUNT as flwMatrix",
...
if _rt is not None and len(_rt) == len(DD["flwMatrix"]):
    ...
    chk(
        "flwMatrixV2 round-trip EXACTLY equals flwMatrix (rows, order, s arrays, u flags)",
        _first_bad is None,
        (
            "identical"
            if _first_bad is None
            else f"first mismatch at row {_first_bad}: {_rt[_first_bad]} vs {DD['flwMatrix'][_first_bad]}"
        ),
    )
else:
...
chk(
    "no '|' or ':' in any FLW id / cohort id (encoding separators stay unambiguous)",
    not any(("|" in r["f"] or ":" in r["f"] or "|" in r["c"] or ":" in r["c"]) for r in DD["flwMatrix"]),
    "",
)
chk("every flwMatrix state value is a single digit 0-9", all(0 <= v <= 9 for r in DD["flwMatrix"] for v in r["s"]), "")
...
chk(
    "render payload is smaller than the full dashboard payload",
    _rd_kb < _dd_kb,
...
print(
    f"    history: {len(_rg_hist)} prior run(s)"
    + (
        f", most recent {_rg_hist[-1].get('date')} ({_rg_hist[-1].get('git_sha')})"
        if _rg_hist
...
chk(
    "no day-over-day regression (monotonic counters, arm tolerance, stall, absolute floors)",
    not _rg_fails,
    (
        (
            f"{len(_rg_fails)} violation(s): "
            + " | ".join(_rg_fails[:8])
            + (f" | ...and {len(_rg_fails) - 8} more" if len(_rg_fails) > 8 else "")
        )
        if _rg_fails
        else f"checked against {len(_rg_hist)} run(s) of history"
    ),
)
...
if (P + F) < _MIN_CHECKS:
    print("")
    print(
        f"  *** ONLY {P + F} CHECKS RAN, EXPECTED >= 180 - a section is silently "
