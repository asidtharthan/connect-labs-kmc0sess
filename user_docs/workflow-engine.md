...
### Viewing an already-created audit run

When you click through from the Program Audit Creator into a per-opportunity audit run that has already had its audits created, the audit creation controls (date-window picker, sampling rates, and Create button) are replaced by a short read-only summary — for example, **"Audits created — view only · window · N sessions"**. This makes it immediately clear that the run is already under way and that no further setup is needed.
