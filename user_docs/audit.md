...
- **Image De-duplication** — a checkbox option that runs a duplication check across all images in the session. When enabled, Labs compares images submitted by the same field worker on the same day and flags any that appear to be duplicates of each other. Each field worker's photos are checked as a separate batch (up to 40 photos per field worker per day), so in a multi-worker audit every field worker's photos are reliably checked — not just those who happened to be processed first. Flagged images receive a **Duplicate** tag on their image tile. In the bulk assessment view, images are automatically sorted so that suspected duplicates appear together in groups, making it easier to compare them side by side and confirm or reject each flag. Leave this option unticked if you do not need de-duplication checking for the session.
...
- **Visit Clustering** — an optional filter available in the Weekly Dual-Track Image Audit workflow, shown alongside the existing Audit Window and Sampling rate settings. When enabled, it groups consecutive visits by the same field worker that are close together in time and/or GPS location. This is useful for spotting likely duplicate or re-photographed measurements. Visit Clustering never changes which images are included in the audit — it only adds a **N Visit Clusters** button next to each row in the session. Clicking that button expands a panel below the audit tile showing, on a single row, the cluster summary, a text list of image IDs in that cluster (for example, `[1667955, 1667962, ...]`), and a **Download CSV** link for each cluster. The exported CSV includes the **Visit ID** (the number shown on the bulk assessment page, such as #1677989) for each image — Beneficiary Name is not included. When the bulk assessment page opens, any image whose visit fell into a visit cluster has **Duplicate/Fake** pre-selected automatically, saving you a step. This pre-tagging never overrides an image that has already been reviewed by a human or AI — only images with no verdict yet are pre-tagged. If both Visit Clustering checkboxes are left unticked, nothing changes from the standard workflow.
...
The Visit Clustering tile includes an additional option: **Send groupings to the Duplicate Detection API**. When you turn this on, each cluster of visits already identified as close together in time and/or location is checked against an image-similarity service. Images that are confirmed as near-duplicates are:
...
The duplicate detection check runs separately for each field worker — each worker's photos are assessed as their own batch rather than combined with everyone else's. This means the per-day photo limit applies per field worker, so on a busy day with multiple workers in the audit, every worker's photos are checked rather than only those who happened to be processed first.

The raw response from the duplicate-detection API is saved against each visit cluster, so there is a full record of why images were or were not flagged — useful if you later need to investigate why a particular image was not caught.
