...
def _wa_row(opp_id, entity_id, building_count):
    return {"opportunity_id": opp_id, "entity_id": entity_id, "building_count": building_count}
...
def test_run_default_excludes_rows_outside_window(MockWDA):
    ...
    fetch_instance = mock.Mock()
    fetch_instance.get_pipeline_data.return_value = {"hsd_visits": {"rows": hsd_rows}, "work_areas": {"rows": []}}
...
def test_run_default_defaults_window_to_yesterday_wat(MockWDA):
    ...
    definition = _make_definition([1001])
    fetch_instance = mock.Mock()
    fetch_instance.get_pipeline_data.return_value = {"hsd_visits": {"rows": []}, "work_areas": {"rows": []}}
    opp_instance = mock.Mock()
    run = mock.Mock()
    run.id = "run-1001"
    opp_instance.create_run.return_value = run
