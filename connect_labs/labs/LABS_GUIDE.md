...
1.  **Run with local settings** (NOT labs settings):
    ...
    **Important**: Use `config.settings.local` (the default), NOT `config.settings.labs_aws`. The `labs_aws` settings are only for the AWS deployment at `labs.connect.dimagi.com`. The `local.py` settings already have all labs middleware configured.
...
```python
from connect_labs.labs.integrations.connect.cli import (
    get_oauth_token,
...
- **Audit App**: `connect_labs/audit/` - Complete implementation
- **Integration Test**: `connect_labs/audit/run_audit_integration.py` - CLI usage example
- **API Client**: `connect_labs/labs/integrations/connect/api_client.py` - Full API reference
